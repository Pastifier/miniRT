...
/*   Updated: 2024/10/07 16:04:08 by ebinjama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool		parse_light(t_program *context, t_split *fields, int curr_line)
{
	...
	if (context->runtime_error == 2 && *(next + context->flt_operations + (temp < 0)) != ',')
	{
	...
	temp = ft_atof((next += context->flt_operations + 1 + (temp <= -0.f)), context);
