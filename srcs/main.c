...
int	main(int argc, char *argv[])
{
	...
	t_vector3 pixel_00_location = vec3_add(viewport_upper_left, vec3_scaleby(vec3_add(context.camera.pixel_delta_u, context.camera.pixel_delta_v), 0.5));
	pixel_00_location.y = -pixel_00_location.y;
	vec3_print(pixel_00_location);
	for(int y = 0; y < WIN_HEIGHT; y++)
	{
		for(int x = 0; x < WIN_WIDTH; x++)
		{
			t_vector3 pixel_center = vec3_add(pixel_00_location, vec3_add(vec3_scaleby(context.camera.pixel_delta_u, x), vec3_scaleby(context.camera.pixel_delta_v, y)));
